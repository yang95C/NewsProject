...
import com.yg.newsproject.baselibs.utils.PermissionPageUtil
        ...
        PermissionDialog.newBuilder()
            ...
            ?.setOnConfirmClickListener {
                if (showPermission) {
                    ...
                    PermissionPageUtil.gotoPermission(
                        activity
                    )
                }
            }
    ...
    fun checkLocServicePermission(activity: FragmentActivity, callback: (() -> Unit)? = null) {
        if (!isLocServiceEnable(activity)) { // 检测是否开启定位服务
            PermissionDialog.newBuilder()
                ...
                .build()
                ?.setOnConfirmClickListener {
                    PermissionPageUtil.gotoLocServicePage(
                        activity
                    )
                }
